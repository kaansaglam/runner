...
public class HackettPriceInfoDeleter {

	private static final int BULK_SIZE = 1000;
	private static final Logger logger = LogManager.getLogger(HackettPriceInfoDeleter.class);

	public static void main(String args[]) {
		new HackettPriceInfoDeleter().execute();
	}
	...
	private void execute() {
		try (Stream<String> stream = Files.lines(Paths.get("/home/galaksiya/hackett-13-oc.json"),
			...
			try {
				stream.forEach(line -> {
					...
					String priceCurrency = extractPriceCurrency(areaServedArray);
					if (priceCurrency.equals("EUR") || priceCurrency.equals("eur")) {
	...
	private String extractPriceCurrency(JsonArray areaServedArray) {
		...
		if (offersEl != null && !offersEl.isJsonNull()) {
			...
			JsonElement priceCurrencyEl = offersArray.get(0).getAsJsonObject().get("priceCurrency");
			if (priceCurrencyEl != null && !priceCurrencyEl.isJsonNull()) {
				priceCurrency = priceCurrencyEl.getAsString();
			}
		}
